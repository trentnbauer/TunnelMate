...
def test_stops_at_first_gap():
    env = base_env(
        **{
            "HOSTNAME_3": "gap.example.com",
            "SERVICE_3": "http://gap:80",
            "ACCESS_3": "public",
        }
    )
...
def test_parses_multiple_sequential_hostnames():
    env = base_env(
        **{
            "HOSTNAME_2": "auth.example.com",
            "SERVICE_2": "http://auth:8080",
            "ACCESS_2": "auth",
            "AUTH_USERS_2": "a@example.com, b@example.com",
        }
    )
    configs = parse_hostnames(env)
...
def test_invalid_accesstype_raises():
    with pytest.raises(ConfigError, match="ACCESS_1"):
        parse_hostnames(base_env(ACCESS_1="admin-only"))


def test_auth_without_authusers_raises():
    with pytest.raises(ConfigError, match="AUTH_USERS_1"):
        parse_hostnames(base_env(ACCESS_1="auth"))
...
def test_duplicate_hostname_raises():
    env = base_env(
        **{
            "HOSTNAME_2": "app.example.com",
            "SERVICE_2": "http://other:80",
            "ACCESS_2": "public",
        }
    )
...
def test_parse_global_requires_all_fields():
    ...
    cfg = parse_global(
        {"CLOUDFLARE_API_TOKEN": "t", "CLOUDFLARE_ACCOUNT_ID": "a", "NAME": "n"}
    )
